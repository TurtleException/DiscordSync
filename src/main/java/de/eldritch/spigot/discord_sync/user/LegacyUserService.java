...
/**
 * Manages all cross-platform {@link LegacyUser} objects.
 */
public class LegacyUserService {
    private final YamlConfiguration userConfiguration;
    private final YamlConfiguration blockConfiguration;

    private final LegacyUserMap userMap = new LegacyUserMap();

    public LegacyUserService() throws IOException, InvalidConfigurationException {
        // initialize config
    ...
    public void reloadUsers() {
        ...
        for (long turtle : turtles) {
            try {
                ...
                LegacyUser user = new LegacyUser(turtle, minecraft, discord, name);
                if (userMap.put(user))
                    stats[0]++;
                else
                    stats[1]++;
    ...
    public void saveUsers() {
        ...
        for (LegacyUser user : userMap.getUserView()) {
    ...
    public LegacyUser getUser(long turtle) {
        LegacyUser user = userMap.get(turtle);
    ...
    public LegacyUser getUserBySnowflake(long snowflake) {
        LegacyUser user = userMap.getBySnowflake(snowflake);
    ...
    public LegacyUser getUserByUUID(UUID uuid) {
        LegacyUser user = userMap.getByUUID(uuid);
    ...
    private @NotNull LegacyUser newUser(@Nullable OfflinePlayer minecraft, @Nullable Member member, @Nullable String name) {
        LegacyUser user = new LegacyUser(EntityBuilder.TurtleBuilder.newID(), minecraft, member, name);
        userMap.put(user);
    ...
    public void register(@NotNull LegacyUser user, Member member) {
        final LegacyUser oldUser = getUserBySnowflake(member.getIdLong());
    ...
    public void register(@NotNull LegacyUser user, OfflinePlayer player) {
        final LegacyUser oldUser = getUserByUUID(player.getUniqueId());
